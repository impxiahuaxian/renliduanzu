...
function changepic() {
	console.log(document.getElementById('workExperience').value)
...
function changchuan(){
	...
	formData.append('jobDesire', document.getElementById('jobDesire').value);
    formData.append('file', document.getElementById('file').files[0]);
    $.ajax({
        type: "post",
        url: "http://127.0.0.1:8080/user/resume/update",
        data: formData,
        contentType: false,
        processData: false,	
...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (data) {
            ...
            $(data.preWorkList).each(function (key, val) {
                content.works.push({
                	id: val.userId,
                    title: val.title,
                    place: val.workCity,
            ...
            $(data.listHrApp).each(function (key, val) {
                content.HrApps.push({
                    title: val.title,
                    place: val.workCity,
...
function infoUpdate() {
    ...
    $.ajax({
        type: "post",
        url: "http://127.0.0.1:8080/user/info/update",
...
//根据行对象获取所在行
	...
	    console.log(trNo+1);
	var x=document.getElementById('test1').rows[trNo].cells;
	str_1 = x[4].innerHTML;
    ...
    $.ajax({
        type:"post",
        url: "http://127.0.0.1:8080/user/refuse",
        data:{positionId:id},
        dataType:"text",
        success: function(msg){
            console.log(msg);
...
function posAdd(obj){
    ...
    $.ajax({
        type:"post",
        url: "http://127.0.0.1:8080/user/plan/"+id,
        dataType:"text",
        success: function(msg){
            console.log(msg);
            window.location.href="/usepaper?id="+msg
        },
        error: function(msg){
            console.log(msg);
        }

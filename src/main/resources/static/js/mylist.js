...
var update = document.getElementsByClassName("updata");
var deletea = document.getElementsByClassName("delete");
var jie = document.getElementsByClassName("jie");
var jieguo = document.getElementsByClassName("jiehuo");
...
function getlist() {
	...
	xhr2.open("post", "../getpaperList.action", true);
...
for (let j = 0; j < 10; j++) {
	deletea[j].onclick = function() {
		...
		var xhr = createxmlreuqest();
		xhr.open("post", "../deletePaper.action", true);
...
for(let h=0;h<10;h++){
	update[h].onclick=function(){
		var id = sessionStorage.ids.split(',');
		upqu[h].href="/updatequestion?id="+id[h];
	}
}
for(let c=0;c<10;c++){
	jie[c].onclick=function(){
		var id = sessionStorage.ids.split(',');
		jieguo[c].href="/result?id="+id[c];
	}
}
for(let k=0;k<10;k++){
	tittle[k].onclick=function(){
		var id = sessionStorage.ids.split(',');
		tittle[k].href="/usepaper?id="+id[k];
	}
}

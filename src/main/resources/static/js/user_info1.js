...
$(document).ready(function () {
    $.ajax({
        url: "http://127.0.0.1:8080/hr/info",
        type: "post",
        dataType: "json",
        success: function (data) {
            console.log(data);
            console.log(data.applyPosList);
            $(data.applyPosList).each(function (key, val) {
                content.boxs.push({
                    ...
                    mobile: val.mobile,
                    email: val.email,
                    workid: val.positionId,
                    state: "申请待处理"
                });
            });
            	$(data.workinglist).each(function (key, val) {
            		content.works.push({
                	id: val.userId,
                	name: val.name,
                    graduation: val.graduation,
            ...
            $(data.positions).each(function (key, val) {
             ...
             */
            //账户手机号
            $("#tel").val(data.hr.hrMobile);
            $("#password").val(data.hr.hrPassword);
            //邮箱
            $("#email").val(data.hr.hrEmail);
            //实名
            $("#inputName").val(data.hr.hrName);
            //性别
            $("#sex").val(data.hr.hrSex);
            $("#school").val(data.hr.hrAddress);
...
function infoUpdate() {
    ...
    $.ajax({
        type: "post",
        url: "http://127.0.0.1:8080/hr/info/update",
...
function posRemove(obj){
    ...
    $.ajax({
        type:"post",
        url: "http://127.0.0.1:8080/hr/refuse/"+id+"/"+workid,
        dataType:"text",
        success: function(msg){
           if(msg == false){
        	   alert("不能拒绝")
...
function posAgree(obj){
    ...
    $.ajax({
        type:"post",
        url: "http://127.0.0.1:8080/hr/agree/"+id+"/"+workid,
        dataType:"text",
        success: function(msg){
            console.log(msg);
            window.location.href="http://127.0.0.1:8080/renwu/agreesuccess?id="+id+"&workid="+workid;
        },
        error: function(msg){
            console.log(msg);
        }
...
function posEnd(obj){
	...
	console.log(str_1);
	workid = x[7].innerHTML;
	workid = workid.replace(/\s*/g,"");
	console.log(workid);
	  }
	 }
	window.location.href='./pingjia?id='+ id +'&name='+str_1+'&workid='+workid;
}
function posSee(obj){
	var id;
	var str_1;
    ...
    $.ajax({
        type:"post",
        url: "http://127.0.0.1:8080/hr/plan/"+id+"/"+str_1,
        dataType:"text",
        success: function(msg){
            console.log(msg);
            window.location.href='/result?id='+msg;
//            window.location.href="http://127.0.0.1:8080/renwu/agreesuccess?id="+id+"&workid="+workid;
        },
        error: function(msg){
            console.log(msg);

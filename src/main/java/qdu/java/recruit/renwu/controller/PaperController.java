...
public class PaperController {
	...
	public @ResponseBody ResponseMessage addPaper(@RequestBody Paper paper, HttpServletRequest request) {
	...
	public @ResponseBody ResponseObject getpaperList(int index, HttpServletRequest request) {
		HREntity hr = (HREntity) request.getSession().getAttribute("hr");
		List<Paper> list = service.getUserList(hr.getHrId(), index);
		ResponseObject response = new ResponseObject();
		if (list != null) {
			response.setFlag(true);

...
//        PageInfo<PositionCompanyBO> pageInfo = new PageInfo<>(recList);       0-6 6-12 12-18
     ...
     */
    @Override
    public List<UserEntity> userPosition(HREntity hr, int page, int limit) {

        //所有用户列表
    	ArrayList<UserEntity> posList = new ArrayList<>();
        posList = userMapper.listUser();

        
        LOGGER.debug("Exit recPosition method");

        return posList.subList(limit * page - limit, limit * page);
    }
    ...
    @Override
    public boolean savePosition(PositionEntity positionEntity) {
        return positionMapper.savePosition(positionEntity);
    }
	@Override
	public List<PositionEntity> listHRApp(int userId) {
		// TODO Auto-generated method stub
		
		return positionMapper.listHRApp(userId);
	}

...
public interface PositionMapper {
    ...
    @Insert("insert into position (title,requirement,quantity,workCity,salaryUp,salaryDown,releaseDate,validDate,statePub," +
            ...
            "#{departmentId},#{categoryId},#{hrIdPub})")
    boolean savePosition(PositionEntity positionEntity);
    @Select("select * from position where positionId IN (select positionId from hrapplication where userId = #{userId})")
    ArrayList<PositionEntity> listHRApp(@Param("userId") int userId);
}
